...
public class GameInfo implements Serializable {
    ...
    public GameInfo(GameInfo original) { //copy constructor
        ...
        this.players = new ArrayList(original.getPlayers());
    }
    ...
    public Player removePlayer(int playerID) throws GameException {
        int index = 0;
        for (Player player : players) {
        ...
        if (index >= this.getPlayersInGame()) {
            //already removed
            return null;
        } 
        this.version++;
    ...
    public int getGameSize() {
        return this.gameSize; //final simple
    }
    
    public boolean isFull() {
        return this.getPlayersInGame() == this.getGameSize(); //simple
    }
