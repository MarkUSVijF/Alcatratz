...
public interface RMI_Client2Server extends Remote {
    ...
    //local read
    public Set<GameInfo> getGames(int port)
            throws RemoteException, ConnectionException;
    
    public GameInfo getGameInfo(int port, int gameID)
            throws RemoteException, ConnectionException, GameException;
    ...
    public GameInfo hostGame(int port, int requestNr, int players, String playerName)
            throws RemoteException, ConnectionException, GameException;//throws exception: to few/many players

    public GameInfo joinGame(int port, int requestNr, int gameID, String playerName)
            throws RemoteException, ConnectionException, GameException;//throws exception: already full, no such game, player name already used

    public void leaveGame(int port, int requestNr)
            throws RemoteException, ConnectionException, GameException;//throws exception: already full, no such game

    public void endGame(int port, int requestNr, ArrayDeque<MoveInfo> lastMoves)
            throws RemoteException, ConnectionException, GameException;

    public String unregisterClient(int port, int requestNr)
            throws RemoteException, ConnectionException, GameException;
}
