...
public class ToClient {
    ...
    public static boolean playerChange(final String connection, final Game game) {
        logger.log(Level.INFO, "{0} -> playerChange to {1}", new Object[]{Thread.currentThread().getName(), connection});

        try {
            RMI_Server2Client proxy = generateStubTo(connection);
            proxy.playerChange(game.getGameInfo());
            logger.log(Level.INFO, "{0} -> playerChanged at {1}", new Object[]{Thread.currentThread().getName(), connection});
            return true;
        } catch (RemoteException ex) {
        logger.log(Level.INFO, "{0} -> could not change player at {1}", new Object[]{Thread.currentThread().getName(), connection});
            return false;
        }
    }

    public static boolean gameStart(final String connection, final Game game) {
        logger.log(Level.INFO, "{0} -> start game at {1}", new Object[]{Thread.currentThread().getName(), connection});
        try {
            RMI_Server2Client proxy = generateStubTo(connection);
            proxy.gameStart(game.getGameInfo(), game.getConnections(), game.getPlayerID(connection));
            logger.log(Level.INFO, "{0} -> game started at {1}", new Object[]{Thread.currentThread().getName(), connection});
            return true;
        } catch (RemoteException ex) {
        logger.log(Level.INFO, "{0} -> game could not be started at {1}", new Object[]{Thread.currentThread().getName(), connection});
            return false;
        }
    }
    ...
    static void construct() throws RemoteException, MalformedURLException {
        ...
        Set<String> ips = Server.adresses();
        ips.retainAll(getIPv4());
        if (ips.isEmpty()) {
