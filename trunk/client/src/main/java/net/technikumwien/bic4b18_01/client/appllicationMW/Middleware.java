...
public class Middleware {
    ...
    public static void construct(RMI_Server2Client s2c) {
        ...
        while (true) {
            myConnection = ToServer.construct(s2c);
            try {
                GameConnection.construct(myConnection);
                break;
            } catch (RemoteException | MalformedURLException ex) {
                //sollte nicht passieren - ziehmlich unmöglich
                // wurde alles scho mal in ToServer.construct gemacht
                logger.log(Level.SEVERE, "{0} -> local registry error [HOW?]", Thread.currentThread().getName());
                ToServer.destruct();
                try {
                    int port = Integer.parseInt(myConnection.substring(myConnection.indexOf(':') + 1));
                    try {
    ...
    public static void destruct() {
        ToServer.destruct();
        GameConnection.destruct(myConnection);
        int port = Integer.parseInt(myConnection.substring(myConnection.indexOf(':') + 1));
        try {
            UnicastRemoteObject.unexportObject(LocateRegistry.getRegistry(port), true);
        ...
        System.clearProperty("sun.rmi.transport.connectionTimeout");
    }
}
