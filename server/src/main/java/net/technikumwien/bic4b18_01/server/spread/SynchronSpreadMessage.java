...
public class SynchronSpreadMessage {
    ...
    public static void process(SpreadMessage sm) throws SpreadException {
        switch (sm.getType()) {
            ...
            case Short.MAX_VALUE:
                processGetServerState(sm);
                break;
            default:
                logger.log(Level.INFO, "{0} -> #{1} received unknown synchron message from {2}.", new Object[]{Thread.currentThread().getName(), Spread.getServerID().toString(), sm.getSender().toString()});
        }
    }
    ...
    private static void processRegisterClientMessage(SpreadMessage sm) throws SpreadException {
        ...
        logger.log(Level.INFO, "{0} -> register client {1} at {2}.", new Object[]{Thread.currentThread().getName(), MessageFormat.format("{0,number,#}", cUID), clientConnection} );
        //
        try {
            ...
            logger.log(Level.INFO, "{0} -> new Request for {1} with NR {2}.", new Object[]{Thread.currentThread().getName(), clientConnection, MessageFormat.format("{0,number,#}", cUID)});
        }
        //Methoden Logic (Body)
        ...
        if (ClientRequests.existsClient(clientConnection)) {
            ...
            current_response = new Response(cUID, new ConnectionException("port already taken"), "ConnectionException", "registerClient");
        } else {
    ...
    private static void processHostGameMessage(SpreadMessage sm) throws SpreadException {
        ...
        logger.log(Level.INFO, "{0} -> host game for {1} with size gameSize{2}.", new Object[]{Thread.currentThread().getName(), clientConnection, gameSize});
        //
        try {
            ...
            logger.log(Level.INFO, "{0} -> new Request for {1} with NR {2}.", new Object[]{Thread.currentThread().getName(), clientConnection, requestNr});
        }
        //Methoden Logic (Body)
        Response current_response;
        try {
    ...
    private static void processJoinGameMessage(SpreadMessage sm) throws SpreadException {
        ...
        String playerName = (String) digest.get(4);
        logger.log(Level.INFO, "{0} -> join game {1} with name {2}.", new Object[]{Thread.currentThread().getName(), gameID, playerName});
        
        //schaun ob der server unseren Request schon abgearbeited hat (duplizierte nachrichten in spread durch client netz ausfall & repeat of request)
        try {
        ...
        try {
            ...
            HashSet<String> recipients=game.getConnections();
            if(game.isFull()){
    ...
    private static void processLeaveGameMessage(SpreadMessage sm) throws SpreadException {
        ...
        String clientConnection = (String) digest.get(1);
        int requestNr = (int) digest.get(2);
        logger.log(Level.INFO, "{0} -> client at {1} tries to leave his game.", new Object[]{Thread.currentThread().getName(), clientConnection});
        //
        try {
    ...
    private static void processEndGameMessage(SpreadMessage sm) throws SpreadException {
        ...
        logger.log(Level.INFO, "{0} -> client {1} won his game.", new Object[]{Thread.currentThread().getName(), clientConnection});
        //
        try {
    ...
    private static void processUnregisterClientMessage(SpreadMessage sm) throws SpreadException {
        ...
        String clientConnection = (String) digest.get(1);
        int requestNr = (int) digest.get(2);
        logger.log(Level.INFO, "{0} -> client at {1} tries to unregister.", new Object[]{Thread.currentThread().getName(), clientConnection});
        //
        try {
    ...
    //##########################################################################
    // response is got by private msg
    static void sendGetServerState() {
        SpreadMessage message = new SpreadMessage();
        message.setType((short) Short.MAX_VALUE);
        try {
            message.digest("update Me");
            deliver(message);
        } catch (SpreadException ex) {
            logger.log(Level.SEVERE, null, ex);
        }
    ...
    private static void processGetServerState(SpreadMessage sm) {
        
        //generate a server update
        
        logger.log(Level.INFO, "{0} -> message of type getServerState.", Thread.currentThread().getName());
        Middleware.serverStateLock.writeLock().lock();
        try {
            sm.getSender();
            SpreadMessage message = new SpreadMessage();
            message = ClientRequests.generateUpdate(message);
            message = CallBackManager.generateUpdate(message);
            message = GameList.generateUpdate(message);
            message.setType((short) Short.MAX_VALUE);
            PrivatSpreadMessage.deliverTo(message, sm.getSender());
        } finally {
